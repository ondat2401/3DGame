...
public class Player : MonoBehaviour
{
    ...
    [Header("Item Info")]
    [HideInInspector] public bool jetpacking = false;
    [HideInInspector] public bool isDrainingEnergy = false;
    public JetpackBootster jetpackBooster;
    ...
    private void SkillManager()
    {
        ...
        if (playerManager.currentEnergy >= 100 && !isDrainingEnergy)
        {
            ...
            if (randomChoice == 0)
                itemManager.JumpBooster(skillTime, 15);
            else
                itemManager.MagnetBooster(skillTime);
            isDrainingEnergy = true;
        }

        if (isDrainingEnergy)
        {
            playerManager.currentEnergy -= skillTime * Time.deltaTime;

            if (playerManager.currentEnergy <= 0)
            {
                playerManager.currentEnergy = 0;
